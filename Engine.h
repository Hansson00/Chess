...
public:
	...
	//Returns hash
	uint64_t make_move(Position* pos, uint32_t move, uint64_t curr_hash);
	...
	uint64_t search(int depth, Position* pos, uint64_t hash);
...
private:
	...
	uint64_t en_passant(Position* pos, int pushedPawn, uint64_t hash);
